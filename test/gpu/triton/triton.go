...
func New(ctx context.Context, server llmutil.Server, logger testutil.Logger) (*Triton, error) {
	started := time.Now()
	llm := &Triton{
		logger: logger,
		server: server,
...
func jsonGet[Out any](ctx context.Context, llm *Triton, endpoint string) (*llmutil.APIResponse[Out], error) {
...
func jsonPost[In, Out any](ctx context.Context, llm *Triton, endpoint string, input In) (*llmutil.APIResponse[Out], error) {
